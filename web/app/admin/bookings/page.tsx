...
function BookingsContent() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-3xl font-semibold text-navy-900">Bookings</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <form onSubmit={handleSearch} className="flex gap-2">
          <Input
            placeholder="Ref, experience or email…"
            value={search}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {bookings.map((booking) => (
            <div
              key={booking.id}
              className="flex items-center justify-between rounded-xl bg-white px-5 py-4 shadow-sm ring-1 ring-sand-200"
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3">
                  <p className="font-medium text-navy-900 truncate">{booking.experience_title}</p>
                  <span className="shrink-0 font-mono text-xs text-navy-400">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-navy-500">
                  ...
                  <span className="flex items-center gap-1">
                    <MapPin className="h-3.5 w-3.5" /> {booking.experience_city}
                  </span>
                  <span className="flex items-center gap-1">
                    <Users className="h-3.5 w-3.5" /> {booking.num_participants}
                  </span>
                </div>
              </div>
